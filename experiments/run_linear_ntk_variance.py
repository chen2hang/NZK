#!/usr/bin/env python3
"""
Linear ZOGD with different Gaussian variances for z (NZK).
Reproduces linear_zo_fo_ntk_variance.
"""
...
def main():
    ...
    parser.add_argument("--variances", type=float, nargs="+", default=[0.5, 1.0, 1.5])
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--out_dir", type=str, default="output_linear_ntk_variance")
    ...
    f_diff_zo_dict = {}
    NZK_dict = {}
    for v in args.variances:
        nzk = build_nzk_linear(
            data_sample, n_samples=args.nzk_samples, dist="normal", scale=v, device=device
        )
        NZK_dict[str(v)] = nzk
        f_0_zo = data_sample @ theta_init
        loss_zo, f_zo_list, f_diff_zo, _ = train_zo_loop_with_f0(
            f_0_zo, target_gt, nzk, args.eta_zo, sample_num,
            args.iteration_total, record_list, device
        )
        f_zo_dict[str(v)] = f_zo_list
        loss_zo_dict[str(v)] = loss_zo
        f_diff_zo_dict[str(v)] = f_diff_zo
    ...
    output_folder = os.path.join(args.out_dir, f"variance_mixture_degree_{args.degree}")
